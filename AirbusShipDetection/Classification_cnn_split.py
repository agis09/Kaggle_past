...
print(labels['has_ship'].value_counts())

# ランダムにunder-sampling
low_data = labels[labels['has_ship']==1]
high_data_index = labels[labels['has_ship']==0].index
random_indices = np.random.choice(high_data_index, len(low_data),replace=False)
high_data = pd.DataFrame(labels.loc[random_indices])
merged_data = pd.concat([high_data,low_data],ignore_index=True)
balanced_data = pd.DataFrame(merged_data)

print(balanced_data['has_ship'].value_counts())
balanced_data['path'] = balanced_data['ids'].map(lambda x: os.path.join(train_image_dir, x))
train_df,valid_df = train_test_split(balanced_data,
                                    test_size=0.2,
                                    shuffle=True,
                                    stratify=balanced_data['has_ship'])
...
fig.savefig('./classification_result_'+BASE_MODEL+'.png')
plt.close()

ship_model.load_weights(weight_path)
ship_model.save('full_ship_model_'+BASE_MODEL+'.h5')
