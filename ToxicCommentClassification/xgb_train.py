...
# print(check_output(["ls", "F:/Shiga/ToxicClassification/xgb_input"]).decode("utf8"))
...
transform_com = TfidfVectorizer(min_df=3,  max_features=None,
                                strip_accents='unicode', analyzer='word', token_pattern=r'\w{1,}',
                                ngram_range=(1, 3), use_idf=1, smooth_idf=1, sublinear_tf=1,
                                stop_words = 'english').fit(train['comment_text'])
...
subm = pd.read_csv('F:/Shiga/ToxicClassification/sample_submission.csv')
submid = pd.DataFrame({'id': subm["id"]})
submission = pd.concat([submid, pd.DataFrame(preds, columns=col)], axis=1)
submission.to_csv('F:/Shiga/ToxicClassification/xgb.csv', index=False)