...
X_train_tmp = np.zeros((len(X_train), IMG_HEIGHT, IMG_WIDTH, 1), dtype=np.float32)
X_test_tmp = np.zeros((len(test_ids), IMG_HEIGHT, IMG_WIDTH, 1), dtype=np.float32)
for i,x in enumerate(X_train):
    x = (x+1)*127.5
    x = x.astype(np.uint8)
    x = cv2.cvtColor(x, cv2.COLOR_RGB2GRAY)
    X_train_tmp[i]=watershed(x)
for i,x in enumerate(X_test):
    x = (x+1)*127.5
    x = x.astype(np.uint8)
    x = cv2.cvtColor(x, cv2.COLOR_RGB2GRAY)
    X_test_tmp[i]=watershed(x)
X_train = X_train_tmp
X_test=X_test_tmp
