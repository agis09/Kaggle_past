...
X_train = np.zeros((len(train_ids)*6, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS), dtype=np.float32)
Y_train = np.zeros((len(train_ids)*6, IMG_HEIGHT, IMG_WIDTH, 1), dtype=np.float32)
...
for n, id_ in tqdm(enumerate(train_ids), total=len(train_ids)):
    ...
    X_train[n+len(train_ids)*4]=img[:,::-1,:]
    ...
    Y_train[n+len(train_ids)*4]=mask[:, ::-1, :]

    (X_train[n+len(train_ids)*5], Y_train[n+len(train_ids)*5]) = scale_augmentation_crop(img,mask)
