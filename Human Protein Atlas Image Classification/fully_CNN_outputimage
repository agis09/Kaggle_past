...
def getTrainDataset(train,full):
    ...
    for name, lbl in tqdm(zip(data['Id'], data['Target'].str.split(' '))):
        ...
        if full==False:
            if cnt > 1 and train == True:
                continue
            elif cnt < 2 and train == False:
                continue

        im_name=os.path.join(path_to_train, name)
        paths.append(im_name)
        img_g = cv2.imread(im_name + '_green.png', cv2.IMREAD_GRAYSCALE)

        cv2.imwrite("./image_sample/"+name+".png",img_g)
        ...
        # cv2.imwrite("./image_sample/" + name + "_mask.png", img_g*255.)
        img_g=cv2.resize(img_g,(32,32))

        # img_g=np.reshape(img_g,(32,32,1))
        ...
        for i in range(1,28):
            if y[i]>0:
                im=np.dstack((im,img_g))
            else:
                im = np.dstack((im,img_tmp))
        # print(im.shape)
        labels.append(im)
        # labels.append(y)
...
class ProteinDataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, paths, labels, batch_size, shape, shuffle=False, use_cache=True, augment=True):
    ...
    def __load_image(self, path):
        ...
        Y = cv2.imread(path + '_yellow.png', cv2.IMREAD_GRAYSCALE)
...
for lr in lr_list:
    ...
    #############################
    # fine_tuning
    # model = load_model('./best.model', custom_objects={'f1': f1,'weighted_binary_crossentropy':weighted_binary_crossentropy}) #, 'f1_loss': f1_loss})
    #############################
    ...
    paths, labels = getTrainDataset(train=True,full=False)
    ...
    checkpoint = ModelCheckpoint('./next_base.model%f'%lr, monitor='val_loss', verbose=1, save_best_only=True,
                                 save_weights_only=False, mode='min', period=1)
    # checkpoint = ModelCheckpoint('./best1.model', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='min', period=1)
    ...
    ax[0].legend()
    ax[1].legend()
    plt.savefig('fcnn_model_Adam%f.png'%lr)



"""

bestModel = load_model('./next_base.model', custom_objects={'f1': f1,'weighted_binary_crossentropy':weighted_binary_crossentropy}) #, 'f1_loss': f1_loss})
# bestModel = load_model('./best1.model', custom_objects={'f1': f1,'weighted_binary_crossentropy':weighted_binary_crossentropy}) #, 'f1_loss': f1_loss})
# bestModel = load_model('./base2.model', custom_objects={'f1': f1,'weighted_binary_crossentropy':weighted_binary_crossentropy}) #, 'f1_loss': f1_loss})
#bestModel = model


pathsVal, labelsVal = getTrainDataset(train=False,full=True)
